...
public class HOCONSerializer extends AbstractIndentAwareSerializer {

    private static final Logger logger = LoggerFactory.getLogger(HOCONSerializer.class);
    ...
    private static void nanWarn() {
        if (!nanWarningIssued) {
            logger.warn("HOCON cannot serialize NaN and infinity values by spec! We'll work-around this by special strings...");
    ...
    protected void writeString(String value, Writer writer) throws IOException {
        String escapedValue = escapeMachine.escape(value);

        if (HOCONHelper.strRequiresQuotes(escapedValue)) {
            writer.write("\"");
    ...
    protected void walkSerializeMap(Map<String, IConfigNode> map, Writer writer, ISerializationContext ctx) throws IOException {
        ...
        while (iterator.hasNext()) {
            ...
            if (HOCONHelper.strRequiresQuotes(key)) {
                writer.write("\"");
                writer.write(key);
                writer.write("\":");
            } else {
